...
Check if the target is the middle element, check if the left or right side is
sorted then check if the target is within the sorted side's range. Then we run
binary search until we find the target element's index or we return `-1` if we
make it through and don't find the target element.
